...
function ProdTshirts() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch(`${apiUrl}/api/TodosProdutos/ListadeTshirts`);
...
<div>
  ...
  <h2 class="text-5xl font-extrabold text-center mb-6 py-8 bg-gradient-to-r from-red-500 to-blue-500 text-white rounded-lg">T-SHIRTS</h2>
            ...
            {product.img ? (
              <div class="group">
                ...
                <div class="absolute bottom-0 left-0 right-0 px-6 py-4 opacity-0 transition-opacity duration-300 bg-black bg-opacity-75 text-white group-hover:opacity-100">
                  ...
                  <p class="text-gray-300 mb-2">Preço: {product.price.toFixed(2)} €</p>
                </div>
              </div>
            ) : (
